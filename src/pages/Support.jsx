...
import { FaSearch, FaExclamationCircle, FaCheckCircle, FaInfoCircle } from "react-icons/fa";
...
const Support = () => {
  ...
  const tstyle = {
    tableClass: "min-w-full bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-gray-200 overflow-hidden text-gray-700",
    headerClass: "bg-gradient-to-r from-blue-600 via-blue-500 to-blue-700 text-white text-sm font-semibold uppercase tracking-wide shadow-inner sticky top-0",
    rowClass: "bg-white even:bg-gray-50 hover:bg-indigo-50/60 transition-all duration-300 shadow-sm hover:shadow-md rounded-xl mb-2 cursor-pointer",
    cellClass: "py-3 px-4 text-sm font-medium first:rounded-l-xl last:rounded-r-xl",
    paginationClass: "bg-white/60 shadow-inner rounded-lg px-4 py-2 text-gray-700 flex items-center justify-center gap-2 mt-4",
  };
  ...
  return (
    ...
    (<div className="p-6 flex flex-col gap-8 bg-gradient-to-br from-gray-50 via-indigo-50 to-purple-50 min-h-screen">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full lg:w-1/2 bg-white/90 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-indigo-200 hover:scale-[1.02] transition-all duration-300">
        ...
        <div className="flex flex-col gap-4">
          ...
          <input
            ...
            className={`border border-gray-300 px-3 py-2 rounded-lg w-full focus:ring-2 focus:ring-indigo-400 outline-none ${idInput.trim() !== "" ? "bg-gray-200 cursor-not-allowed" : ""}`}
          />
          ...
          <div className="flex gap-3">
            <input
              ...
              onChange={handleChange}
              disabled={idInput.trim() !== ""}
              className={`border border-gray-300 px-3 py-2 rounded-lg w-1/2 focus:ring-2 focus:ring-indigo-400 outline-none ${idInput.trim() !== "" ? "bg-gray-200 cursor-not-allowed" : ""}`}
            />
            <input
              type="date"
              name="endDate"
      ...
      {/* Table */}
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white/90 p-5 rounded-2xl shadow-lg border border-gray-200 backdrop-blur-lg">
        ...
        {loading ? (
          ...
          <Table columns={columns} data={tableData} rowsPerPage={5} isPaginationRequired={true} {...tstyle} />
        ) : (
