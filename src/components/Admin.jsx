...
const Admin = () => {
  ...
  useEffect(() => {
    if (merchantsData) {
      setTopUsers(Array.isArray(merchantsData.top_users) ? merchantsData.top_users : []);
      setLatestTransactions(Array.isArray(merchantsData.successful_reports) ? merchantsData.successful_reports : []);
      setMonthlyEarnings(Array.isArray(merchantsData.month_wise_earnings) ? merchantsData.month_wise_earnings : []);
...
const userRows = merchantsData?.top_users?.map((user) => ({
...
const mappedTransactions = merchantsData?.successful_reports?.map((item) => ({
  id: item.id,
...
const chartCategories = lastSixMonths?.map((item) => {
  const [year, month] = item.month_year.split("-");
  return `${monthNames[parseInt(month, 10) - 1]}-${year}`; // e.g., "Dec-2025"
}) || [];

const dataArray = lastSixMonths?.map((item) => parseFloat(item.total_earning)) || [];
